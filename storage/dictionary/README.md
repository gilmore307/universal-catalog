...
The seed bootstraps the register with field entries for the `catalog_items` column names themselves, base repo/path/config entries, and the first ratified shared workflow slot fields for execution-key, completion-receipt, and acceptance-receipt templates.
