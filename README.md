...
This repository currently defines the documentation spine, the PostgreSQL register skeleton, and an initial read-only Node helper surface under `src/`. The seed now includes the catalog register's own column names, base repo/path/config entries, and the first ratified shared workflow slot fields for execution-key, completion-receipt, and acceptance-receipt templates.
